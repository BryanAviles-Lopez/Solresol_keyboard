...
def get_midi_port():
    for name in mido.get_input_names():
        if 'BK-5' in name:  # Check for Roland BK-5 in the list of MIDI devices
            return name
    return None
...
def main():
    port_name = get_midi_port()

    if port_name is None:
        print("Roland BK-5 not found.")
        return
        ...
        for msg in inport:
            if msg.type == 'note_on':
                ...
                if note:
                    ...
                    # Convert Solresol syllables to lowercase before lookup
                    word = solresol_to_english_word(note.lower())
                    if word:
